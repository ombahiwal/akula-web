...
const handler = async (request: Request, context: any) => {
  ...
  // First try by sys.id
  endpoints[0].searchParams.set("content_type", "eventsTimeline");
  ...
  // Then try slug field
  endpoints[1].searchParams.set("content_type", "eventsTimeline");
  ...
  const title = stringifyField(fields.eventTitle || fields.title, "AKULA Event");
  const description = truncate(
    stringifyField(
      fields.eventDescEn || fields.description || stripRichText(fields.eventArticle) || "",
      "Join us for this event."
    ),
    200
  );

  // Events can have multiple images in an array
  const imageField = fields.images || fields.image || fields.thumbnail;
  let image = fallbackImage;
  
  if (Array.isArray(imageField) && imageField.length > 0) {
    image = resolveAssetUrl(imageField[0], assets) || fallbackImage;
  } else {
    image = resolveAssetUrl(imageField, assets) || fallbackImage;
  }

  const canonical = `${siteUrl}${url.pathname}`;
  const published = fields.eventDate || entry.sys.createdAt || new Date().toISOString();

  const html = `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8" />
<title>${escapeHtml(title)} · AKULA Event</title>
